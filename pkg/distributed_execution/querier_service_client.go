...
// NewQuerierPool creates a connection pool of clients to other queriers, used by the
// Remote operator to pull intermediate results from queriers executing child fragments.
...
func floatHistogramProtoToFloatHistograms(hps []cortexpb.Histogram) []*histogram.FloatHistogram {
	floatHistograms := make([]*histogram.FloatHistogram, 0, len(hps))
	for _, hp := range hps {
