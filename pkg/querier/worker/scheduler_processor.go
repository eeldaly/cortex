...
func (sp *schedulerProcessor) runRequest(ctx context.Context, logger log.Logger, queryID uint64, frontendAddress string, statsEnabled bool, request *httpgrpc.HTTPRequest, isRoot bool) {
	...
	// For distributed execution, only the root fragment reports the result back to the
	// frontend. Non-root fragments expose their result to parent queriers via the
	// querier-to-querier gRPC server (backed by the QueryTracker) instead.
	if !isRoot {
		return
	}
